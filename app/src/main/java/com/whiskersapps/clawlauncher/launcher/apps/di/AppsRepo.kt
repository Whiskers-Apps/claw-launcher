...
class AppsRepo(
    ...
    private suspend fun updateShortcuts() {
        ...
        * */
        withContext(Dispatchers.Main) {
            ...
            val asyncShortcuts = async(Dispatchers.IO) {
                ...
                packageManager.queryIntentActivities(intent, 0).forEach { appIntent ->
                    if (appIntent.activityInfo.packageName != "com.whiskersapps.clawlauncher") {
                        ...
                        val shortcuts: List<App.Shortcut> = try {
                        ...
                        newApps.add(
                            App(
                                ...
                                shortcuts = if (shortcuts.size > 4) shortcuts.subList(
                                    0,
                                    4
                                ) else shortcuts
                            )
            ...
            val hiddenApps = settingsRepo.settings.value.hiddenApps
    ...
    fun openShortcut(packageName: String, shortcut: App.Shortcut) {
